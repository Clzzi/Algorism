...
#define MAX_PNLEN 200 // 최대 전화번호 길이는 11자 ( Max PhoneNumber Length )
...
void Init() {
	...
	for (int i = 0; i < MAX_STU; i++) // 학생 수는 최대 200명
...
void AddStu() { // 학생 추가 및 학생 데이터 입력 함수
	...
	while (1) { // 이름을 잘못입력할시 제대로된 이름을 받을때까지 반복함
		printf("  이름 : ");
		scanf("%s%c", sName); // sName에 이름을 입력받음
	...
	for (int n = 0; n < MAX_SUB; n++) {
		...
		scanf("%d", p_stu->scores + n); // p_stu->scores는 주소임, 받을때마다 주소값을 더해주면 그다음주소로 이동해 점수를 받게됨.
...
int IsVaildScore(int score) {
	// 받은 점수가 1 ~ 100 사이라면 즉, 조건둘다 맞을시 1(true) , 아니면 0(false) 리턴
...
int duplicatePNum(char* sPn) { // sPn을 받아와서 중복 검사해줌. 
	for (int i = 0; i <= MAX_NUM; i++) { // 3321( MAX_NUM )까지 반복문을 돌려서
